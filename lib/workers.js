            ...
            checks.forEach(check => {
                ...
                _data.read('checks', check, function(err, originalCheckData) {
                    if (!err && originalCheckData) {
                        ...
                        console.error('\x1b[31m%s\x1b[0m', "Error reading one of the check's data :" + JSON.stringify(err));
                    }
                })
            })
        } else {
            console.error('\x1b[31m%s\x1b[0m', "Error: Could not find any checks to process.");
        }
    });
}
...
workers.validateCheckData = function(originalCheckData) {
    ...
    if (originalCheckData.id &&
        ...
        console.error('\x1b[31m%s\x1b[0m', "Error one of the checks is not properly formatted. Skipping it.");
    }
}
...
workers.processCheckOutcome = function(originalCheckData, checkOutcome)  {
    ...
    _data.update('checks', newCheckData.id, newCheckData, function(err) {
        if(!err) {
            ...
            console.error('\x1b[31m%s\x1b[0m', "Error trying to save updates to one of the checks.");
        }
    })
}
...
workers.log = function(originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) {
    ...
    _logs.append(logFileName, logString, function(err) {
        if(!err) {
            ...
            console.error('\x1b[31m%s\x1b[0m', "Logging to file failed");
        }
    })
}
                    ...
                    if(!err) {
                        ...
                        _logs.truncate(logId, function(err) {
                            if(!err) {
                                ...
                                console.error('\x1b[31m%s\x1b[0m', "Error truncating log file");
                            }
                        })
                    } else {
                        console.error('\x1b[31m%s\x1b[0m', "Error compressing one of the log files, " + JSON.stringify(err));
                    }
                })
            })
        } else {
